...
- Preto je tu kontrola FAST_MODE + TOP_K_FEATURES, aby si vedela mať 2 režimy:
...
Nechávam aj metriky, ktoré sú zatiaľ NaN – budú sa hodiť pre Anchor/PDP.
"""
...
# Anchor je výpočtovo náročný pri veľa featurách – preto menší beam a limit vzoriek
RUN_ANCHOR = True
...
ANCHOR_DELTA = 0.15  # tolerancia odhadu precision v Anchor

# LIME sanity check – trénuje "random-label" LGBM s malým počtom stromov
RUN_LIME_SANITY_CHECK = True
SANITY_LGBM_TREES = 60 if FAST_MODE else 150

# čitateľnejšie výpisy tabuliek v konzole
...
SHOW_PLOTS = False
RUN_SINGLE_PATIENT_DEMO = False   # ak nechceš grafy a detailný výpis jedného pacienta
RUN_LIME_SANITY_CHECK = False     # pri 20 pacientoch radšej vypnúť
# =========================================================
...
# 2. krok – zvyšných 85 % rozdelíme na 70 % train a 15 % val
# podiel validačnej časti z X_temp: 0.15 / 0.85 ≈ 0.17647
...
print(" Trénujem LightGBM...")
model.fit(X_train, y_train)
print(" Model natrenovaný.")

# 2. KLASICKÉ METRIKY MODELU
def predict_proba_df(model, X, feature_names):
    import pandas as pd
...
def _predict_proba(model, X_any):
    ...
    return model.predict_proba(pd.DataFrame(X_any, columns=feature_names))
...
model_margins = model.predict(x_patient_df, raw_score=True)[0]
...
def lime_monotonicity(
    ...
    base_pred = model.predict_proba(x_instance.reshape(1, -1))[0][pred_class]
    ...
    for feat_str, w in weights.items():
        ...
        new_pred = model.predict_proba(x_new.reshape(1, -1))[0][pred_class]
...
if RUN_LIME_SANITY_CHECK:
    ...
    (lime_sanity_rho, lime_sanity_fid_orig, lime_sanity_fid_rand), lime_sanity_time = _timeit(
        ...
        pred_class, num_features=LIME_NUM_FEATURES, num_samples=LIME_NUM_SAMPLES, n_estimators=SANITY_LGBM_TREES
    )
...
exp_dalex = dx.Explainer(
    ...
    label=f"LightGBM_prob_class_{pred_class}"
)
    ...
    for op in ops:
        if op in cond:
            ...
            try:
                ...
                # fallback pre textové kategórie
...
try:
    out_xlsx = "lightgbm_xai_metriky_vysledky.xlsx"

    xai_table.to_excel(out_xlsx, index=False)

    print(f"\ Exportované: {out_xlsx}")
