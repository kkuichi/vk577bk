...
- Preto je tu kontrola FAST_MODE + TOP_K_FEATURES:
  (1) FAST_MODE=True  -> rýchla, stále rozumná aproximácia (top-k featury, menší bootstrap)
  (2) FAST_MODE=False -> robustnejšie, ale výrazne pomalšie (viac featur, viac replikácií)
"""
...
SANITY_XGB_TREES = 60 if FAST_MODE else 150
...
plt.xlabel("Predikovaná trieda")
plt.ylabel("Skutočná trieda")
plt.title("Confusion Matrix (normalizovaná)")
plt.tight_layout()
plt.show()
...
# 3. JEDEN PACIENT NA VYSVETLENIE
patient_idx = 0
...
print(f"\n Pacient #{patient_idx}")
#print(f"Predikovaná trieda (index): {pred_class} | label: {pred_label} | pravdepodobnosti: {probs}")
#print(f"Skutočná trieda: {true_class} | label: {true_label}")
...
def _predict_proba(model, X_any):
    ...
    return model.predict_proba(np.asarray(X_any))
...
model_margins = model.predict(x_patient_df, output_margin=True)[0]
...
print(f"   reconstructed probs = {shap_probs_reconstructed}")
print(f"   model probs         = {probs}")
print(f"   max abs prob error  = {shap_prob_error:.6e}")
print(f"   max abs margin err  = {shap_margin_error:.6e}")
#shap.plots.waterfall(shap_exp_single, max_display=15)
#plt.show()
...
for c in range(num_classes):
    imp_c = pd.DataFrame({
        "feature": feature_names,
        "mean_abs_shap": mean_abs_by_class[:, c]
    }).sort_values("mean_abs_shap", ascending=False)
...
shap_global_rank_mean_rho, shap_global_rank_std_rho = bootstrap_spearman_ranking(
    abs_shap_global_per_sample, rounds=BOOTSTRAP_ROUNDS
)
...
shap_global_rob = shap_global_robustness(
    explainer_shap,
    X_global,
    noise_scale=0.01,
...
shap_global_mono_mean, shap_global_mono_std = shap_global_monotonicity(
    X_shap_global_mono,
    shap_signed_vals,
    top_features
)
...
if TUNE_LIME_KERNEL_WIDTH:
    best_score, best_w, lime_exp = tune_lime_kernel_width(
        X_train_np, feature_names, class_names, x_patient, model.predict_proba,
...
def lime_importance_dict(lime_exp,pred_class):
    #return {k: float(v) for k, v in lime_exp.as_list()}
...
#def lime_stability(explainer_lime, model, x_instance, noise_scale=0.01, trials=10, num_features=10, num_samples=8000):
def lime_stability(explainer_lime, model, x_instance, pred_class,noise_scale=0.01, trials=10, num_features=10, num_samples=8000):
    ...
    #base_dict = dict(base_exp.as_list())
...
def lime_monotonicity(
    ...
    for feat_idx, w in pairs:
        ...
        if X_ref is not None:
            ...
            x_new[feat_idx] = float(x_instance[feat_idx]) + delta_frac

        new_pred = float(model.predict_proba(x_new.reshape(1, -1))[0][pred_class])
        delta_pred = new_pred - base_pred

        if abs(delta_pred) < eps:
            continue

        score = float(np.sign(w) == np.sign(delta_pred))
        scores.append(score)
...
def lime_sanity_check_random_labels(X_train, y_train, X_instance, feature_names, class_names, kernel_width, pred_class, num_features=10, num_samples=8000, n_estimators=60, seed=42):
    ...
    exp_orig = expl.explain_instance(X_instance, model.predict_proba, labels=[int(pred_class)], num_features=num_features, num_samples=int(num_samples))
    exp_rand = expl.explain_instance(X_instance, m_rand.predict_proba, labels=[int(pred_class)], num_features=num_features, num_samples=int(num_samples))

    #d_orig = dict(exp_orig.as_list())
    #d_rand = dict(exp_rand.as_list())
...
if RUN_LIME_SANITY_CHECK:
    ...
    (lime_sanity_rho, lime_sanity_fid_orig, lime_sanity_fid_rand), lime_sanity_time = _timeit(
        ...
        pred_class, num_features=LIME_NUM_FEATURES, num_samples=LIME_NUM_SAMPLES, n_estimators=SANITY_XGB_TREES
    )
...
def dalex_predict_proba_pred_class(model, data):
    ...
    return model.predict_proba(data)[:, pred_class]
...
exp_dalex = dx.Explainer(
    ...
    label=f"XGBoost_prob_class_{pred_class}"
)

bd = exp_dalex.predict_parts(
    x_patient_df,
    type="break_down"
)
...
# dalex môže mať buď 'variable_name' alebo 'variable' podľa verzie
...
'''
def bd_explain_fn(x):
    obs = pd.DataFrame([x], columns=feature_names)
    bd = exp_dalex.predict_parts(obs, type="break_down")
    return dict(zip(bd.result["variable"], bd.result["contribution"]))
'''
    ...
    for r_idx in range(MAX_ANCHOR_RULES):
        ...
        try:
            ...
            if any(tuple(x["rule"]) == rule_tuple for x in anchor_all_rules):
                print(" ️ Našlo sa duplicitné pravidlo. Končím.")
                break
...
dice_gen = Dice(data_dice, model_dice, method="random")

query_patient = X_test.iloc[[patient_idx]]
desired_class_idx = 0

cf = dice_gen.generate_counterfactuals(
    query_patient,
    total_CFs=15,
    desired_class=desired_class_idx
)

cf_df = cf.cf_examples_list[0].final_cfs_df
print("\n Counterfactual príklady:")
print(cf_df)

def print_pretty_counterfactuals(cf_df, df, X_test, patient_idx, target_col, desired_class_idx):
    feature_cols = [c for c in cf_df.columns if c != target_col]
    original_row = df.loc[X_test.index[patient_idx], feature_cols]
    print("\n What-if pravidlá z counterfactualov:\n")
    for i in range(cf_df.shape[0]):
        cf_row = cf_df.iloc[i][feature_cols]
        diff_mask = cf_row != original_row
        changed_cols = [col for col in feature_cols if diff_mask[col]]
        print(f"Counterfactual #{i+1}:")
        if not changed_cols:
            print("  (Žiadna zmena – model už predikuje desired_class.)")
        else:
            print("  IF")
            for col in changed_cols:
                print(f"    - {col}: {original_row[col]}  →  {cf_row[col]}")
            print(f"  THEN model predikuje triedu {desired_class_idx}.")
        print()

print_pretty_counterfactuals(cf_df, df, X_test, patient_idx, TARGET, desired_class_idx)

def cf_metrics(cf_df, original_row, desired_class_idx, model, target_col):
    cf_only = cf_df.drop(columns=[target_col], errors="ignore")
    original_vals = original_row[cf_only.columns].values

    preds = np.argmax(model.predict_proba(cf_only.values), axis=1)
    validity = np.mean(preds == desired_class_idx)

    dists = []
    sparsities = []
    for i in range(cf_only.shape[0]):
        cf_vals = cf_only.iloc[i].values
        diff = cf_vals - original_vals
        dists.append(np.linalg.norm(diff, ord=1))  # L1
        sparsities.append(np.sum(diff != 0))

    return {
        "validity": float(validity),
        "avg_L1_distance": float(np.mean(dists)),
        "avg_sparsity": float(np.mean(sparsities))
    }

cf_stats = cf_metrics(cf_df, df.iloc[X_test.index[patient_idx]], desired_class_idx, model, TARGET)
'''
...
def pdp_trendability(model, X_ref, feature, class_idx,
                     rounds=20, grid_resolution=20, seed=42):
    rng = np.random.default_rng(seed)

    _, full_vals = compute_pdp_for_feature(
    ...
    for _ in range(int(rounds)):
        ...
        _, boot_vals = compute_pdp_for_feature(
            model, X_boot, feature, class_idx, grid_resolution=grid_resolution
        )
...
def pdp_stability_shape_and_value(model, X_ref, feature, class_idx, rounds=10, grid_resolution=20, seed=42):
    ...
    for _ in range(int(rounds)):
        idx = rng.integers(0, n, size=n)
        X_boot = X_ref.iloc[idx]
        _, vals_boot = compute_pdp_for_feature(
            model, X_boot, feature, class_idx, grid_resolution=grid_resolution
        )
...
def pdp_stability(model, X_ref, feature, class_idx, rounds=10, grid_resolution=20, seed=42, eps=1e-12):
    ...
    _, vals_full = compute_pdp_for_feature(
        model, X_ref, feature, class_idx, grid_resolution=grid_resolution
    )
...
def pdp_robustness(model, X_ref, feature, class_idx, noise_scale=0.01, trials=10, grid_resolution=20, seed=42):
    ...
    _, vals_base = compute_pdp_for_feature(
        model, X_ref, feature, class_idx, grid_resolution=grid_resolution
    )
    ...
    for _ in range(trials):
        ...
        noise = rng.normal(0, noise_scale, size=(len(X_noisy), len(num_cols)))
        X_noisy.loc[:, num_cols] = X_noisy[num_cols].to_numpy(dtype=float) + noise
        _, vals_noisy = compute_pdp_for_feature(
            model, X_noisy, feature, class_idx, grid_resolution=grid_resolution
        )
...
if RUN_PDP:
    ...
    for f in top_features[:PDP_TOP_M]:
        (grid, vals), t_pdp = _timeit(
            compute_pdp_for_feature, model, X_train, f, pred_class, PDP_GRID_RESOLUTION
        )

        mono_rho = pdp_monotonicity_spearman(grid, vals)       # ∈ [-1, 1]
        (tr_mean, tr_std), t_tr = _timeit(
            pdp_trendability,
            model, X_train, f, pred_class,
            PDP_BOOT_ROUNDS, PDP_GRID_RESOLUTION, 42
        )
        smooth = pdp_smoothness_var(vals)                      # >=0
        amp = pdp_amplitude(vals)                              # >=0
        lin_r2 = pdp_linearity_r2(grid, vals)                  # ∈ [0, 1] (ideálne)

        # --- stability: shape + value ---
        #stab_dict, t_stab = _timeit(
        #    pdp_stability_shape_and_value,
        #    model, X_train, f, pred_class,
        #    PDP_BOOT_ROUNDS, PDP_GRID_RESOLUTION, 42
        #)
        stab, t_stab = _timeit(
        ...
        ice_var, t_ice = _timeit(
            ice_variance_score,
            model, X_train, f, pred_class, grid,
            ICE_SAMPLE_N, 42
        )
        rob, t_rob = _timeit(
            pdp_robustness,
            model, X_train, f, pred_class,
            0.01, 10 if FAST_MODE else 20, PDP_GRID_RESOLUTION, 42
        )

        pdp_metrics.append({
            "top 5 atributov": f,
            "pdp_efficiency_sec": float(t_pdp), #čas výpočtu samotnej PDP krivky
            #"stability_efficiency_sec": float(t_stab),  #čas bootstrap stability
            #"ice_efficiency_sec": float(t_ice), #čas výpočtu ICE variance
            "stability": float(stab),
            "robustness": float(rob),
            "monotonicity": float(mono_rho), #rho
            "trendability": tr_mean,      #mean_rho
            #"trendability_std_rho": tr_std,
...
trend_mean_rho, trend_std_rho = trendability_spearman(
    X_trend,
    shap_trend_vals,
    top_features,
    use_abs=True
...
#lime_imp = lime_importance_dict(lime_exp)
...
lime_trend_imp = lime_importance_matrix(
    explainer_lime,
    model,
    X_trend_lime,
    pred_class,
...
trend_mean_rho_lime, trend_std_rho_lime = trendability_spearman(
    X_trend_lime,
    lime_trend_imp,
    lime_top_features
)
...
trend_mean_rho_bd, trend_std_rho_bd = trendability_spearman(
    X_trend_bd,
    bd_trend_imp,
    bd_top_features
)
...
rows.append({
    ...
    #"trendability_std_rho": trend_std_rho,
    "necessity": necessity_shap,
    "sufficiency": suff_prob_shap,    #_retained_prob
    "sufficiency_logit": suff_logit_shap,
    #"sufficiency_class_stable": suff_class_shap,
    #"descriptive_accuracy": descriptive_accuracy_shap,
    "explanation_parsimony": parsimony_shap,
    #"simplicity_gini": simp_gini_shap,
    "simplicity": 1 / (1 + simp_entropy_shap),       #_entropy
    "precision": np.nan,
    "coverage": np.nan,
...
rows.append({
    ...
    #"trendability_std_rho": trend_std_rho_lime,
    "necessity": necessity_lime,
    "sufficiency": suff_prob_lime,  #_retained_prob
    "sufficiency_logit": suff_logit_lime,
    #"sufficiency_class_stable": suff_class_lime,
    #"descriptive_accuracy": descriptive_accuracy_lime,
    "explanation_parsimony": parsimony_lime,
    #"simplicity_gini": simp_gini_lime,
    "simplicity": 1 / (1 + simp_entropy_lime),     #_entropy
    "precision": np.nan,
    "coverage": np.nan,
    #"subgroup_fairness_disparity": np.nan,
    #"subgroup_used": None,
    #"application_utility": application_utility_lime,
    #"lime_sanity_rho": lime_sanity_rho if RUN_LIME_SANITY_CHECK else np.nan,
    #"shap_global_rank_mean_rho": np.nan,
    #"shap_global_rank_std_rho": np.nan,
})
...
rows.append({
    ...
    #"trendability_std_rho": trend_std_rho_bd,
    "necessity": necessity_bd,
    "sufficiency": suff_prob_bd,       #_retained_prob
    "sufficiency_logit": suff_logit_bd,
    #"sufficiency_class_stable": suff_class_bd,
    #"descriptive_accuracy": descriptive_accuracy_bd,
    "explanation_parsimony": parsimony_bd,
    #"simplicity_gini": simp_gini_bd,
    "simplicity": 1 / (1 + simp_entropy_bd), #_entropy
    "precision": np.nan,
    "coverage": np.nan,
    #"subgroup_fairness_disparity": np.nan,
    #"subgroup_used": None,
    #"application_utility": np.nan,
    #"lime_sanity_rho": np.nan,
    #"shap_global_rank_mean_rho": np.nan,
    #"shap_global_rank_std_rho": np.nan,
})
...
if RUN_PDP and pdp_df is not None and not pdp_df.empty:
    ...
    #pdp_stability = float(pdp_df["stability_shape_rho_mean"].mean())
    ...
    rows.append({
        ...
        #"trendability_std_rho": pdp_trend_std,
...
# 10B. NORMALIZÁCIA METRÍK – rovnaký smer, vyššie = lepšie
def invert_metric(x):
    if pd.isna(x):
        return np.nan
...
# kópia raw tabuľky
xai_table_norm = xai_table.copy()

invert_cols = ["consistency", "stability", "robustness", "efficiency_sec"]

for col in invert_cols:
    if col in xai_table_norm.columns:
        xai_table_norm[col] = xai_table_norm[col].apply(invert_metric)

if "monotonicity" in xai_table_norm.columns:
    xai_table_norm["monotonicity"] = xai_table_norm["monotonicity"].abs()

if "trendability" in xai_table_norm.columns:
    xai_table_norm["trendability"] = xai_table_norm["trendability"].abs()

if "simplicity" in xai_table_norm.columns:
    xai_table_norm["simplicity"] = xai_table_norm["simplicity"].apply(invert_metric)

if "explanation_parsimony" in xai_table_norm.columns:
    xai_table_norm["explanation_parsimony"] = xai_table_norm["explanation_parsimony"].apply(invert_metric)

for col in xai_table_norm.columns:
    if col == "method":
        continue
    xai_table_norm[col] = minmax_normalize(xai_table_norm[col])

print("\n================= NORMALIZOVANÉ XAI METRIKY =================")
print(xai_table_norm)

metric_cols_for_score = [c for c in xai_table_norm.columns if c != "method"]
xai_table_norm["overall_score"] = xai_table_norm[metric_cols_for_score].mean(axis=1, skipna=True)

print("\n================= OVERALL SCORE =================")
print(xai_table_norm[["method", "overall_score"]])
try:
    out_xlsx = "xai_metriky_vysledky8.xlsx"
    out_xlsx_norm = "xai_metriky_normalizovane8.xlsx"
    xai_table.to_excel(out_xlsx, index=False)
    xai_table_norm.to_excel(out_xlsx_norm, index=False)
    print(f"\n Exportované: {out_xlsx}")
    print(f" Exportované normalizované: {out_xlsx_norm}")
except Exception as e:
