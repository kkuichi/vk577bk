...
- Preto je tu kontrola FAST_MODE + TOP_K_FEATURES, aby bolo možné mať 2 režimy:
...
# Anchor je výpočtovo náročný pri veľa feature
RUN_ANCHOR = True
...
SHOW_PLOTS = False
RUN_SINGLE_PATIENT_DEMO = False   # grafy a detailný výpis jedného pacienta
...
# zvyšných 85 % rozdelíme na 70 % train a 15 % val
...
def _ensure_df(X_any, feature_names):
    """
    CatBoost bol trénovaný na pandas DataFrame s názvami stĺpcov.
    Preto každý numpy vstup pred predikciou prevedieme späť na DataFrame,
    aby nevznikali warningy typu: X does not have valid feature names.
    """
...
def predict_raw_margin_df(model, X_any, feature_names):
    return model.predict(_ensure_df(X_any, feature_names), prediction_type="RawFormulaVal")
...
def model_robustness(model, x_instance, noise_scale=0.01, trials=20):
    """Robustnosť predikcie modelu (nie vysvetlenia): P(zmena triedy pri šume)."""
...
def explanation_parsimony_topk(importance_dict, k=10, threshold=0.95):
    """
    Koľko prvkov vysvetlenie používa:
    - importance_dict: {feature: abs_importance}
    - return: min počet featur potrebných na dosiahnutie threshold podielu sumy abs
    """
...
def simplicity_entropy_and_gini(importance_dict):
    """
    ↓ entropy / ↑ koncentrácia = jednoduchšie vysvetlenie.
    - entropy: Shannon entropy z normalizovaných abs váh
    - gini-like concentration: 1 - sum(p^2) (nižšie znamená viac koncentrované)
    """
    if not importance_dict:
...
# Perturbation-based metriky (zrýchlené top-k)
...
def completeness_drop_auc(
    ...
    # maximálny pokles po zamaskovaní všetkých featur z importance_order
...
def sufficiency_class_stable(model, x_df: pd.DataFrame, top_features, baseline: pd.Series, pred_class: int):
    """Či zostane predikovaná trieda rovnaká po ponechaní len top_features (ostatné baseline)."""
    x_new = _as_float_df(x_df)
    keep = set(top_features)
    for c in x_new.columns:
        if c not in keep:
...
model_margins = predict_raw_margin_df(model, x_patient_df, feature_names)[0]
...
shap_signed_vals = explainer_shap(X_shap_global_mono).values[:, :, pred_class]
...
def tune_lime_kernel_width(X_train_np, feature_names, class_names, x_patient, predict_fn, widths, pred_class, num_features=10,num_samples=8000,seed=42):
...
# heuristika pre kandidátov kernel_width (škálujeme podľa sqrt(#features))
...
def lime_stability(explainer_lime, model, x_instance, pred_class,noise_scale=0.01, trials=10, num_features=10, num_samples=8000):
    base_exp = explainer_lime.explain_instance(data_row=x_instance, predict_fn=lambda z: _predict_proba(model, z),labels=[int(pred_class)],
                                               num_features=num_features,num_samples=int(num_samples))
...
def lime_sanity_check_random_labels(X_train, y_train, X_instance, feature_names, class_names, kernel_width, pred_class, num_features=10, num_samples=8000, n_estimators=60, seed=42):
    ...
    exp_orig = expl.explain_instance(
        X_instance,
        lambda z: _predict_proba(model, z),
        labels=[int(pred_class)],
        num_features=num_features,
    ...
    exp_rand = expl.explain_instance(
        X_instance,
        lambda z: m_rand.predict_proba(_ensure_df(z, feature_names)),
        labels=[int(pred_class)],
        num_features=num_features,
        num_samples=int(num_samples),
    )
...
# 9. TECHNICKÉ METRIKY – rozšírený výpočet (top-k)
...
rows.append({
    ...
    "consistency": np.nan,
    "completeness": np.nan,
    "stability": 1.0 - shap_global_rank_mean_rho,
...
# export do excelu
try:
    out_xlsx = "catboost_xai_metriky_vysledky.xlsx"
    xai_table.to_excel(out_xlsx, index=False)
    print(f"\n✅ Exportované: {out_xlsx}")
except Exception as e:
    print("⚠️ Export do Excel zlyhal:", repr(e))
