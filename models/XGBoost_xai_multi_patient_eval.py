...
def _predict_proba(model: XGBClassifier, X_any) -> np.ndarray:
...
def evaluate_model(model: XGBClassifier, X_test: pd.DataFrame, y_test: np.ndarray) -> Dict[str, object]:
...
def evaluate_single_patient(
    ...
    if run_lime_sanity:
        def lime_sanity_check_random_labels(X_train_, y_train_, X_instance_, feature_names_, class_names_, kernel_width_, pred_class_, num_features=10, num_samples=3000, n_estimators=60, seed=42):
            ...
            m_rand = XGBClassifier(
                ...
                num_class=len(np.unique(y_train_)),
            ...
            exp_orig = expl.explain_instance(X_instance_, model.predict_proba, labels=[int(pred_class_)], num_features=num_features, num_samples=int(num_samples))
            exp_rand = expl.explain_instance(X_instance_, m_rand.predict_proba, labels=[int(pred_class_)], num_features=num_features, num_samples=int(num_samples))
    ...
    # export detail tabuliek jedného pacienta
    try:
...
def main():
    ...
    if RUN_MULTI_PATIENT_EVAL:
        ...
        try:
            out_xlsx = "xai_multi_patient_eval_detail.xlsx"
            out_xlsx_mean = "xai_multi_patient_eval_mean.xlsx"
            out_xlsx_median = "xai_multi_patient_eval_median.xlsx"
            out_xlsx_std = "xai_multi_patient_eval_std.xlsx"
