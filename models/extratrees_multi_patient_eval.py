...
SANITY_EXTRATREES_TREES = 100 if FAST_MODE else 200
...
def _predict_proba(model: ExtraTreesClassifier, X_any) -> np.ndarray:
...
def evaluate_model(model: ExtraTreesClassifier, X_test: pd.DataFrame, y_test: np.ndarray) -> Dict[str, object]:
...
def lime_importance_matrix(
    ...
    for i in range(len(X_ref)):
        ...
        exp = explainer_lime.explain_instance(data_row=x_i,predict_fn=lambda z: _predict_proba(model, z),labels=[int(pred_class)],num_features=num_features,num_samples=int(num_samples), )
...
def build_dalex_explainer(
    ...
    def dalex_predict_proba_pred_class(model_, data):
        ...
        return _predict_proba(model_, data)[:, pred_class]
...
def evaluate_single_patient(
    ...
    shap_prob_reconstructed = float(
        shap_vals_patient.base_values[0, pred_class]
        + np.sum(shap_vals_patient.values[0, :, pred_class])
    )
    shap_prob_error = float(abs(shap_prob_reconstructed - probs[pred_class]))
    shap_fidelity = float(max(0.0, 1.0 - shap_prob_error))

    print("\n SHAP fidelity (probability-scale):", shap_fidelity)
    print("   reconstructed prob =", shap_prob_reconstructed)
    print("   model prob         =", probs[pred_class])
    print("   abs prob error     =", shap_prob_error)
    ...
    if run_lime_sanity:
        def lime_sanity_check_random_labels(X_train_, y_train_, X_instance_, feature_names_, class_names_, kernel_width_, pred_class_, num_features=10, num_samples=3000, n_estimators=60, seed=42):
            ...
            m_rand = ExtraTreesClassifier(n_estimators=int(n_estimators),max_depth=None,random_state=seed,n_jobs=-1,)
            ...
            exp_rand = expl.explain_instance(
                X_instance_,
                lambda z: m_rand.predict_proba(pd.DataFrame(np.asarray(z).reshape(-1, len(feature_names_)), columns=feature_names_)),
...
def evaluate_multi_patient(
    ...
    for patient_idx in eval_patient_indices:
        ...
        shap_vals_patient = explainer_shap(x_patient_df)
        values_for_class = shap_vals_patient.values[0, :, pred_class]
        # ExtraTrees: SHAP rekonštrukcia pre predikovanú triedu na probability škále.
        shap_prob_reconstructed = float(
            shap_vals_patient.base_values[0, pred_class]
            + np.sum(shap_vals_patient.values[0, :, pred_class])
        )
...
def main():
    ...
    if RUN_MULTI_PATIENT_EVAL:
        ...
        try:
            out_xlsx = "ExtraTrees_xai_multi_patient_eval_detail.xlsx"
            out_xlsx_mean = "ExtraTrees_xai_multi_patient_eval_mean.xlsx"
            out_xlsx_median = "ExtraTrees_xai_multi_patient_eval_median.xlsx"
            out_xlsx_std = "ExtraTrees_xai_multi_patient_eval_std.xlsx"
